...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await expect(window, "浅色侧栏使用高对比语义颜色", "(() => { const style = getComputedStyle(document.documentElement); return style.getPropertyValue('--sidebar-text').trim() === '#272a27' && style.getPropertyValue('--sidebar-muted').trim() === '#626762' && style.getPropertyValue('--sidebar-faint').trim() === '#7b817b'; })()");
    ...
    await wait(220);
    await expect(window, "深色外观与默认文稿画布保持一致", "document.documentElement.dataset.appearance === 'dark' && getComputedStyle(document.querySelector('#editor-scroll')).backgroundColor === 'rgb(31, 34, 36)' && getComputedStyle(document.querySelector('#write')).color === 'rgb(221, 226, 229)'");
    await expect(window, "深色侧栏使用高对比语义颜色", "(() => { const style = getComputedStyle(document.documentElement); return style.getPropertyValue('--sidebar-text').trim() === '#f0f1ee' && style.getPropertyValue('--sidebar-muted').trim() === '#bdc1ba' && style.getPropertyValue('--sidebar-faint').trim() === '#9da39b'; })()");
    ...
    await wait(220);
    await expect(window, "切回浅色外观恢复默认文稿纸张", "document.documentElement.dataset.appearance === 'light' && getComputedStyle(document.querySelector('#editor-scroll')).backgroundColor === 'rgb(255, 255, 255)'");
