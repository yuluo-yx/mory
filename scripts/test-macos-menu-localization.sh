...
OUTPUT_PATH="$PROJECT_DIR/.build/mory-mac-menu-localization-smoke"
...
env CLANG_MODULE_CACHE_PATH="$PROJECT_DIR/.cache/clang" SDKROOT="$SDK_PATH" \
  swiftc -sdk "$SDK_PATH" -framework AppKit \
  "$PROJECT_DIR/Sources/Mory/MenuLocalizer.swift" \
  "$PROJECT_DIR/Tests/MacMenuLocalizationSmoke.swift" \
  -o "$OUTPUT_PATH"
