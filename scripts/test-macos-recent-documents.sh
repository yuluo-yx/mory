...
OUTPUT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mory-recent-test.XXXXXX")"
...
env CLANG_MODULE_CACHE_PATH="$PROJECT_DIR/.cache/clang" SDKROOT="$SDK_PATH" swiftc \
  -framework AppKit \
  "$PROJECT_DIR/Sources/Mory/RecentDocuments.swift" \
  "$PROJECT_DIR/Tests/MacRecentDocumentsSmoke.swift" \
  -o "$OUTPUT_DIR/MacRecentDocumentsSmoke"
"$OUTPUT_DIR/MacRecentDocumentsSmoke"
