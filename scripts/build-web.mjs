...
const domPurifyInputPath = path.join(projectDirectory, "node_modules", "dompurify", "dist", "purify.min.js");
const domPurifyOutputPath = path.join(vendorDirectory, "dompurify.min.js");
const domPurifyLicenseInputPath = path.join(projectDirectory, "node_modules", "dompurify", "LICENSE");
const domPurifyLicenseOutputPath = path.join(vendorDirectory, "dompurify.LICENSE");
...
const domPurifyRuntime = await readFile(domPurifyInputPath, "utf8");
const domPurifyLicense = await readFile(domPurifyLicenseInputPath, "utf8");
...
if (process.argv.includes("--check")) {
  ...
  const currentDOMPurify = await readFile(domPurifyOutputPath, "utf8").catch(() => "");
  const currentDOMPurifyLicense = await readFile(domPurifyLicenseOutputPath, "utf8").catch(() => "");
  ...
  if (current !== bundle || currentDOMPurify !== domPurifyRuntime || currentDOMPurifyLicense !== domPurifyLicense
    || currentMermaid !== mermaidRuntime || currentLicense !== mermaidLicense
  ...
  await writeFile(domPurifyOutputPath, domPurifyRuntime, "utf8");
  await writeFile(domPurifyLicenseOutputPath, domPurifyLicense, "utf8");
  ...
  console.log(`Synchronized DOMPurify runtime: ${path.relative(projectDirectory, domPurifyOutputPath)}`);
